'''Реализовать класс Road (дорога), в котором определить атрибуты: length (длина),
 width (ширина). Значения данных атрибутов должны передаваться при создании
  ...
  массы асфальта, необходимого для покрытия всего дорожного полотна. Использовать
...
class Road:
    ...
    def __init__(self, _length, _width):
       self.calculation(_length,_width)

    def calculation(self, _length, _width, mass = 0.025, height = 5 ):
        print(f'Масса асфальта ровняется: {_length * _width * mass * height} т.')
